...
type MonthRangeCalProps = {
	...
	onYearForward?: () => void;
	onYearBackward?: () => void;
	callbacks?: { yearLabel?: (year: number) => string; monthLabel?: (month: Month) => string };
	variant?: { calendar?: { main?: ButtonVariant; selected?: ButtonVariant }; chevrons?: ButtonVariant };
	minDate?: Date;
	maxDate?: Date;
														...
														cn(
															cn(
																...
																(menuYear + m.yearOffset > startYear ||
																	(menuYear + m.yearOffset == startYear && m.number > startMonth)) &&
																	(menuYear + m.yearOffset < endYear ||
																		(menuYear + m.yearOffset == endYear && m.number < endMonth)) &&
															...
															if (
																...
																if (onMonthRangeSelect)
																	onMonthRangeSelect({
																		...
																		end: new Date(menuYear + m.yearOffset, m.number),
																	});
															}
														} else {
															setRangePending(true);
													...
													disabled={
														(maxDate
															? menuYear + m.yearOffset > maxDate?.getFullYear() ||
																(menuYear + m.yearOffset == maxDate?.getFullYear() && m.number > maxDate.getMonth())
															: false) ||
														(minDate
															? menuYear + m.yearOffset < minDate?.getFullYear() ||
																(menuYear + m.yearOffset == minDate?.getFullYear() && m.number < minDate.getMonth())
															: false)
													}
													className={cn(
														buttonVariants({
